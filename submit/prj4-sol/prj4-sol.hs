...
substTerm :: Term -> [Binding] -> Term
substTerm (Var name) bindings =
    case assoc name bindings of
        Just binding -> substTerm binding bindings
        Nothing -> Var name
substTerm (Struct name allterms) bindings =
    Struct name (map (\term -> substTerm term bindings) allterms)
...
normalizeBindings :: [Binding] -> [Binding]
normalizeBindings bindings = sortOn fst $ map normalizeBinding bindings
  where
    normalizeBinding (name, term) = (name, substTerm term bindings)
...
-- Unify two terms and return a list of bindings, or Nothing if the terms cannot be unified.
-- Unify two terms and return a list of bindings, or Nothing if the terms cannot be unified.
unify :: Term -> Term -> Maybe [Binding]
unify t1 t2 = unifyAux t1 t2 []

unifyAux :: Term -> Term -> [Binding] -> Maybe [Binding]
unifyAux (Var x) (Var y) bindings
    | x == y = Just bindings
    | otherwise =
        case lookup x bindings of
            Just b -> unifyAux b (Var y) bindings
            Nothing ->
                case lookup y bindings of
                    Just b -> unifyAux (Var x) b bindings
                    Nothing -> Just $ normalizeBindings ((x, Var y) : bindings)

unifyAux (Var x) t bindings =
    case lookup x bindings of
        Just b -> unifyAux b t bindings
        Nothing -> Just $ normalizeBindings ((x, t) : bindings)
unifyAux t (Var y) bindings = unifyAux (Var y) t bindings
unifyAux (Struct t1 ts1) (Struct t2 ts2) bindings
    | t1 /= t2 = Nothing
    | length ts1 /= length ts2 = Nothing
    | otherwise = unifyList ts1 ts2 bindings

unifyList :: [Term] -> [Term] -> [Binding] -> Maybe [Binding]
unifyList [] [] bindings = Just bindings
unifyList (t1:ts1) (t2:ts2) bindings =
    case unifyAux t1 t2 bindings of
